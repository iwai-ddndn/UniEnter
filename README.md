...
> **前提: 各アプリの送信設定は既定値(Enter=送信)にすること。**
> UniEnterは「Enter→Shift+Enter(改行)」「⌘Enter→素のEnter(送信)」に書き換えるため、
> アプリ側で送信キーを変更していると破綻する。特にSlack/Teamsを「Enterで改行」モードに
> している場合、そのアプリは既に統一挙動(Enter=改行/⌘Enter=送信)なので、
> **UniEnterの対象から外す**(チェックを外す)こと。⌘Enterの意味がモードで反転するため、
> どちらのモードでも通用する書き換えは存在しない。
