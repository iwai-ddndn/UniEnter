> **【廃止】Paddleでの検証は完了せず、Polar.shへ切替済み。以降はこのファイルではなく
> `notes/POLAR-SETUP.md` を参照してください。**(このファイルは経緯の記録として残す)
...
4. ウェブサイト: `https://unienter.oc-to.com/` を申告
...
1. **ドメイン審査(Website approval)**: Checkout → Website approval で
   `https://unienter.oc-to.com/` を申請。
   審査観点は「何を売っているか明確」「利用規約・プライバシーポリシーがある」
   「価格が明示されている」— すべて対応済み。
   - ドメイン欄には `unienter.oc-to.com` を入力(https://やパスは付けない。
     サブドメインは個別申請が必要な仕様)。自社ドメイン化済みなので審査は通りやすいはず
